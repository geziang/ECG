...
rows = [
    ...
    ["D3", "导联轴(关系层)", "共源-特异解耦 z=[c;s], c 跨导联一致、s 去相关、c⊥s", "🟢 M 矩阵候选(模块级,2026-09-16 回归)", "E007 证明两极端皆错,D3 为中间态;塌缩退 D1L", "01 §五 v3.0"],
    ["D4", "时间轴(编码器)", "CNN 保形态 + Mamba 长程节律主干", "❌ 出局(换主干=大结构创新,2026-09-16)", "工程报告路线不做结构创新;除非顺带出正号按普通模块入 M 库", "01 §五 v3.0"],
    ...
    ["D7", "目标轴(视图)", "时间掩码重建支路 L=L_BT+η·L_rec(mask0.5,η0.1)", "🟢 M 矩阵候选(模块级并联支路,期望最高)", "E005 证伪蒸馏式,重建式不同族;写作与 CoRe-ECG 划界", "01 §五 v3.0"],
