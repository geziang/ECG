...
> 文档路径:`runlog/HOSTS.md`;最后更新:**2026-09-17 19:45,主机A(Win4090)**。
>
> **⚠️ 主机A当前状态:⏸ 暂停(19:42,用户指令)**——全部实验进程(链×5/m_screen×2/预训练×2/FT10)已停止,§二/§三 任务仍由A认领但**未在执行**;恢复命令见 §三 末尾。
>
...
**当前无(⏸ 19:42 暂停)。** 今日已完成:`aug_mask_weak` ❌(18:53,−0.60pt)、`aug_crop_085` ❌(18:56,−0.29pt,裁剪曲线闭合)。

**暂停时被中断的半途任务**(恢复后从 epoch 0 重跑,约损失 1.7 GPU 时):`proj_dim1024`(进度 ~45min)、`d7_rec01`(进度 ~45min)、FT10×2(vgg_1d output_dim 已修复,待重跑)。
...
> **主机A恢复方式(一条命令,幂等)**:`pipeline_runner.py --lane C` ——自动跳过已完成任务、认领防撞、显存/槽位双闸门;**勿再重启 chain*.sh 脚本**(19:40 排查发现 chainA/chainB2 各有两份重复接力残留,已全部清除)。
...
截至 09-17 19:00,`matrix_results.csv` 共 15 行判定:**d1l_05_02 ❌ / d1l_neg(负对照)/ n4_ema999 ❌ / aug_crop_strong(0.4)❌ / b0_gamma07 ❌ / b0_gamma09 ❌ / b0_lambd003 ❌ / b0_lambd01 ❌ / aug_crop_weak(s0 +0.27 → s2 ❌ → s4 ❌,3-seed 门判负)/ aug_crop_080 ❌ / aug_crop_090 ❌ / aug_mask_weak ❌(−0.60pt,掩码轴关闭)/ aug_crop_085 ❌(−0.29pt,裁剪曲线 0.4<0.5<0.7峰>0.8>0.85>0.9 闭合)**。另:N1 四库语料 ❌、D9 VICReg 六形态 ❌(详见根目录实验报告)。
