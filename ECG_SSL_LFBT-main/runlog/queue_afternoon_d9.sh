...
run_pair() {
  echo "[$(date +%H:%M)] == D9 确认: $1 seed $2 开始 ==" >> "$SUM"
  "$PY" run_pt.py --data-dir data/pt_pretrain --epochs 200 --batch-size 128 \
    --workers 6 --seed "$2" $3 --checkpoint-dir "checkpoint/s1_$1_seed$2" \
    > "runlog/S1/pt_${1}_seed$2.log" 2>&1 \
    && echo "[$(date +%H:%M)] $1 seed$2 预训练完成" >> "$SUM" \
    || { echo "[$(date +%H:%M)] $1 seed$2 预训练失败" >> "$SUM"; return 1; }
  run_lp "checkpoint/s1_$1_seed$2" "feat/s1_${1}_seed$2" "runlog/S1/lp_${1}_seed$2.log"
}
