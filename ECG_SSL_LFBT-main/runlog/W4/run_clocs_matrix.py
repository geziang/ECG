# -*- coding: utf-8 -*-
"""W4 A-4: CLOCS(S2) 全量矩阵 = NFH 100ep PT × seeds{0,2,4} + 四行下游×3 seeds。

双车道用法: python run_clocs_matrix.py laneA   (seed0 全链 -> seed4 全链)
            python run_clocs_matrix.py laneB   (seed2 全链)
...
HP = "clocs-vs-c1(见runlog/W4/baseline_hparams.csv)"
SEEDS = [0, 2, 4]
    ...
    with open(LOGD / f"{tag}.log", "w", encoding="utf-8") as f:
        rc = subprocess.run(
            [PY, "-u", "run_lp.py", "--data-dir", f"data/{ds}", "--num-classes", str(nc),
             ...
             "--protocol-id", "w4-clocs-baseline"],
            stdout=f, stderr=subprocess.STDOUT, cwd=str(ROOT)).returncode
    m = json.loads((feat / "metrics.json").read_text(encoding="utf-8"))
    record("clocs", seed, "lp", ds, m["auroc"], m["auprc"], ck_sha(ckdir))
    return rc == 0


def run_ft10(seed):
    ckdir = ROOT / f"checkpoint/confirm/clocs_seed{seed}"
    tag = f"clocs_ft10_ptbxl_seed{seed}"
    ...
    record("clocs", seed, "ft10", "ptbxl", m["auroc"], m["auprc"], ck_sha(ckdir))
    return rc == 0
...
def downstream(seed):
    ...
    for ds, nc, ev in jobs:
        if row_done("clocs", seed, ev, ds):
            log(f"  clocs s{seed} {ev}/{ds}: 已有账, 跳过")
            continue
        log(f"  clocs s{seed} {ev}/{ds}: 启动")
        ok = run_ft10(seed) if ev == "ft10" else run_lp(ds, nc, seed)
        if not ok:
            log(f"  clocs s{seed} {ev}/{ds}: FAIL, 停止本 seed 链")
            return False
    return True
...
if __name__ == "__main__":
    ...
    assert lane in LANES, "用法: run_clocs_matrix.py [laneA|laneB]"
